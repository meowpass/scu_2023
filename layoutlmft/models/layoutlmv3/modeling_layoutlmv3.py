...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=(3, patch_size[0], patch_size[1]),
        #                       stride=(1, patch_size[0], patch_size[1]), padding=(1, 0, 0))
    ...
    def forward(self, x, position_embedding=None):
        # # (B,C,H,W) -> (B,C',H',W')
        # # x size (B,C,H,W) -> (1,B,C,H,W) -> (1,C,B,H,W) -> (1,C',B,H',W')
        # x = x.unsqueeze(0)
        # x = x.transpose(1, 2)
...
class LayoutLMv3PageSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
    ...
    def forward(
        ...
        page_inputs = hidden_states.reshape(-1,self.hidden_size)
        page_inputs = page_inputs.unsqueeze(0)
        size = page_inputs.size()
        page_outputs = torch.ones(size=size,device='cuda:0')
        # page_outputs.detach()
        for i in range(3):
            mixed_query_layer = self.query(page_inputs[:,i+1::3,:])
            ...
            if is_cross_attention and past_key_value is not None:
                ...
                key_layer = self.transpose_for_scores(self.key(page_inputs[:,i+1::4,:]))
                value_layer = self.transpose_for_scores(self.value(page_inputs[:,i+1::4,:]))
            ...
            page_outputs[:,i+1::3,:] = context_layer

        # outputs = (context_layer, attention_probs) if output_attentions else (context_layer,)
        page_outputs = page_outputs.squeeze(0)
        page_outputs = page_outputs.reshape(3,-1,self.hidden_size)


        return page_outputs


class LayoutLMv3PageAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = LayoutLMv3PageSelfAttention(config)
        self.output = RobertaSelfOutput(config)
        self.pruned_heads = set()
...
class LayoutLMv3Layer(nn.Module):
    def __init__(self, config):
        ...
        # # TODO
        # self.page_attention = LayoutLMv3PageAttention(config)
        # self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        # # TODO
        # page_self_attention_outputs = self.page_attention(
        #     hidden_states,
        #     None,
        #     head_mask,
        ...
        # )
        #
        # # page_self_attention_outputs = page_inputs.squeeze(0)
        # # page_self_attention_outputs = page_self_attention_outputs.reshape(4,-1,self.hidden_size)
        #
        # # page_attention_outputs = page_self_attention_outputs[0]
        # # page_outputs = page_self_attention_outputs[1:]
        # # page_layer_output = apply_chunking_to_forward(
        # #     self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, page_attention_outputs
        # # )
        # # page_outputs = (page_layer_output,) + page_outputs
        ...
        # attention_output = self_attention_outputs[0] + page_self_attention_outputs
